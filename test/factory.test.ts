...
describe("TREXFactory", () => {
  async function getTREXSuiteDeployedTokenAddress(
    ...
    const event = receipt.logs.find(
      e => (e as EventLog)?.fragment?.name === "TREXSuiteDeployed"
    ) as EventLog;
  ...
  describe(".setIdFactory()", () => {
    describe("when try to input address 0", () => {
      it("should revert", async () => {
        ...
        ).to.be.revertedWithCustomError(trexFactory, "ZeroAddress");
      });
    });
    ...
    describe("when sender is owner and factory owns the trex contract", () => {
      it("should transfer ownership on the desired contract", async () => {
        ...
        expect(tx)
          ...
          .withArgs(trexFactory.target, aliceWallet.address);
