...
describe("ModularCompliance", () => {
  ...
  describe(".bindToken", () => {
    describe("when calling as another account that the owner", () => {
      it("should revert", async () => {
        ...
        await expect(
          compliance.connect(anotherWallet).bindToken(token.target)
        ).to.be.revertedWithCustomError(compliance, "OnlyOwnerOrTokenCanCall");
      });
    });
  ...
  describe(".unbindToken", () => {
    describe("when calling as another account", () => {
      it("should revert", async () => {
        ...
        await expect(
          compliance.connect(anotherWallet).unbindToken(token.target)
        ).to.be.revertedWithCustomError(compliance, "OnlyOwnerOrTokenCanCall");
      });
    });
    ...
    describe("when calling as the owner", () => {
      ...
      describe("when token is not bound", () => {
        it("should revert", async () => {
          ...
          await expect(
            compliance.unbindToken(token.target)
    ...
    describe("when calling as the token given in parameters", () => {
      it("should bind the new compliance to the token", async () => {
        ...
        await expect(tx)
          .to.emit(token, "ComplianceAdded")
          .withArgs(complianceBeta.target);
        await expect(tx)
    ...
    describe("when calling as the owner", () => {
      ...
      describe("when module address is already bound", () => {
        it("should revert", async () => {
          ...
          await expect(
            compliance.addModule(module.target)
      ...
      describe("when module is not plug & play", () => {
        ...
        describe("when compliance is suitable for binding to the module", () => {
          it("should revert", async () => {
            ...
            await compliance.connect(accounts.deployer).bindToken(token.target);
            await token
              .connect(accounts.tokenAgent)
              .burn(accounts.aliceWallet.address, 1000);
            await token
              .connect(accounts.tokenAgent)
              .burn(accounts.bobWallet.address, 500);
      ...
      describe("when attempting to bind a 25th module", () => {
        it("should revert", async () => {
          ...
          await Promise.all(
            modules.map(module => compliance.addModule(module.target))
          );
          ...
          await expect(
            compliance.addModule(module.target)
          ).to.be.revertedWithCustomError(compliance, "MaxModulesReached");
        });
      });
    });
  });
    ...
    describe("when calling as the owner", () => {
      ...
      describe("when module address is not bound", () => {
        it("should revert", async () => {
          ...
          await expect(
            compliance.removeModule(module.target)
      ...
      describe("when module is bound", () => {
        it("should remove the module", async () => {
          ...
          await expect(tx)
            ...
            .withArgs(moduleB.target);

          await expect(compliance.isModuleBound(moduleB.target)).to.be
            .eventually.false;
        });
      });
    });
    ...
    describe("when calling as a bound token", () => {
      ...
      describe("when amount is zero", () => {
        it("should revert", async () => {
          ...
          ).to.be.revertedWithCustomError(compliance, "ZeroValue");
        });
      });
  ...
  describe(".supportsInterface()", () => {
    it("should return false for unsupported interfaces", async () => {
      ...
      expect(
        await compliance.supportsInterface(unsupportedInterfaceId)
      ).to.equal(false);
    });
    ...
    it("should correctly identify the IModularCompliance interface ID", async () => {
      ...
      const iModularComplianceInterfaceId =
        await interfaceIdCalculator.getIModularComplianceInterfaceId();
      expect(
        await compliance.supportsInterface(iModularComplianceInterfaceId)
      ).to.equal(true);
    });
    ...
    it("should correctly identify the IERC3643Compliance interface ID", async () => {
      ...
      const iComplianceInterfaceId =
        await interfaceIdCalculator.getIERC3643ComplianceInterfaceId();
      expect(
        await compliance.supportsInterface(iComplianceInterfaceId)
      ).to.equal(true);
    });
    ...
    it("should correctly identify the IERC173 interface ID", async () => {
      ...
      expect(await compliance.supportsInterface(ierc173InterfaceId)).to.equal(
        true
      );
    });
    ...
    it("should correctly identify the IERC165 interface ID", async () => {
      ...
      expect(await compliance.supportsInterface(ierc165InterfaceId)).to.equal(
        true
      );
    });
  });
