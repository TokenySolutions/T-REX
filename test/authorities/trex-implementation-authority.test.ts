        ...
        describe("When the trex factory to add is not using this authority contract", () => {
          it("Should revert for invalid link between the factory and the authority", async () => {
            ...
            const contractsStruct = {
              ...
              ctrImplementation:
                implementations.claimTopicsRegistryImplementation.target,
              irImplementation:
                implementations.identityRegistryImplementation.target,
              irsImplementation:
                implementations.identityRegistryStorageImplementation.target,
              tirImplementation:
                implementations.trustedIssuersRegistryImplementation.target,
              mcImplementation:
                implementations.modularComplianceImplementation.target,
            };
            ...
            await expect(
              trexImplementationAuthority.setTREXFactory(trexFactory.target)
        ...
        describe("When the trex factory to add is using this authority contract", () => {
          it("should set the trex factory address", async () => {
            ...
            await expect(tx)
              ...
              .withArgs(trexFactory.target);
            await expect(
              trexImplementationAuthority.getTREXFactory()
            ).to.eventually.equal(trexFactory.target);
          });
        });
      });
    });
        ...
        describe("When the trex factory to add is using this authority contract", () => {
          it("should set the trex factory address", async () => {
            ...
            const tx = await trexImplementationAuthority.setIAFactory(
              implementationAuthorityFactory.target
            );
            await expect(tx)
              .to.emit(trexImplementationAuthority, "IAFactorySet")
              .withArgs(implementationAuthorityFactory.target);
          });
        });
      });
    });
        ...
        describe("when caller is not owner of the token (or any contract of the suite)", () => {
          it("should revert", async () => {
            ...
            await expect(
              trexImplementationAuthority
                .connect(anotherWallet)
                .changeImplementationAuthority(token.target, ethers.ZeroAddress)
            ).to.be.revertedWithCustomError(
              trexImplementationAuthority,
        ...
        describe("when version used by the reference contract is not the same as currently deployed implementations", () => {
          it("should revert", async () => {
            ...
            await expect(
              trexImplementationAuthority.changeImplementationAuthority(
                token.target,
                otherTrexImplementationAuthority.target
              )
            ).to.be.revertedWithCustomError(
              trexImplementationAuthority,
    ...
    describe("TREXImplementationAuthority", () => {
      describe(".supportsInterface()", () => {
        it("should return false for unsupported interfaces", async () => {
          ...
          expect(
            await trexImplementationAuthority.supportsInterface(
        ...
        it("should correctly identify the ITREXImplementationAuthority interface ID", async () => {
          ...
          const iTREXImplementationAuthorityInterfaceId =
            await interfaceIdCalculator.getITREXImplementationAuthorityInterfaceId();
          expect(
            await trexImplementationAuthority.supportsInterface(
              iTREXImplementationAuthorityInterfaceId
            )
          ).to.equal(true);
        });
    ...
    describe("IAFactory", () => {
      describe(".supportsInterface()", () => {
        it("should return false for unsupported interfaces", async () => {
          ...
          const iaFactory = await ethers.deployContract(
            "IAFactory",
            [trexFactory.target],
            deployer
          );
          ...
          const unsupportedInterfaceId = "0x12345678";
          expect(
            await iaFactory.supportsInterface(unsupportedInterfaceId)
          ).to.equal(false);
        });
        ...
        it("should correctly identify the IIAFactory interface ID", async () => {
          ...
          const iIAFactoryInterfaceId =
            await interfaceIdCalculator.getIIAFactoryInterfaceId();
          expect(
            await iaFactory.supportsInterface(iIAFactoryInterfaceId)
          ).to.equal(true);
        });
        ...
        it("should correctly identify the IERC165 interface ID", async () => {
          ...
          const ierc165InterfaceId =
            await interfaceIdCalculator.getIERC165InterfaceId();
          expect(
            await iaFactory.supportsInterface(ierc165InterfaceId)
          ).to.equal(true);
        });
      });
    });
