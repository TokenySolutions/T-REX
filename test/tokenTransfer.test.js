...
contract("Token", accounts => {
    ...
    it("Transfer from passes after unfreezing tokens", async () => {
        ...
        let frozenTokens2 = await token.frozenTokens(user1);
        await token.approve(accounts[4], 300, {from: user1}).should.be.fulfilled;
        await token.transferFrom(user1, user2, 300, {from: accounts[4]}).should.be
            .fulfilled;
        let balance = await token.balanceOf(user1);
        frozenTokens1.toString().should.equal("800");
        frozenTokens2.toString().should.equal("300");
    ...
    it("Forced transfer successful between frozen addresses", async () => {
        ...
        await token.forcedTransfer(user1, user2, 300, {from: agent}).should.be
            .fulfilled;
        let balance1 = await token.balanceOf(user1);
        let balance2 = await token.balanceOf(user2);
        balance1.toString().should.equal("700");
    ...
    it("Forced transfer fails if exceeds partial freeze amount", async () => {
        await token.freezePartialTokens(user1, 800, {from: agent});
        await token
            .forcedTransfer(user1, user2, 300, {from: agent})
            .should.be.rejectedWith(EVMRevert);
        let balance1 = await token.balanceOf(user1);
        let balance2 = await token.balanceOf(user2);
        balance1.toString().should.equal("1000");
    ...
    it("Should proceed a batch of 2 mint transactions", async () => {
        let tx = await token.batchMint([user1, user2], [1000, 1000], {from: agent}).should
            .be.fulfilled;
    ...
    it("Should proceed a batch of 20 mint transactions", async () => {
        let tx = await token.batchMint([user1, user1, user1, user1, user1, user1, user1, user1, user1, user1, user1, user1, user1, user1, user1, user1, user1, user1, user1, user1],
            [20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20], {from: agent}).should
            .be.fulfilled;
    ...
    it("batchMint should fail if not called by an agent", async () => {
        await token.batchMint([user1, user2], [1000, 1000], {from: user1}).should
    ...
    it("Should proceed a batch of 2 burn transactions", async () => {
        await token.batchMint([user1, user2], [1000, 1000], {from: agent}).should
            .be.fulfilled;
        let tx = await token.batchBurn([user1, user2], [1000, 1000], {from: agent}).should
            .be.fulfilled;
    ...
    it("Should proceed a batch of 20 burn transactions", async () => {
        let tx = await token.batchBurn([user1, user1, user1, user1, user1, user1, user1, user1, user1, user1, user1, user1, user1, user1, user1, user1, user1, user1, user1, user1],
            [20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20], {from: agent}).should
            .be.fulfilled;
    ...
    it("batchBurn should fail if not called by an agent", async () => {
        await token.batchMint([user1, user2], [1000, 1000], {from: agent}).should
            .be.fulfilled;
        await token.batchBurn([user1, user2], [1000, 1000], {from: user1}).should
            .be.rejectedWith(EVMRevert);
        let balance1 = await token.balanceOf(user1);
        let balance2 = await token.balanceOf(user2);
        balance1.toString().should.equal("2000");
    ...
    beforeEach(async () => {
        ...
        //user1 gets signature from claim issuer
        hexedData3 = await web3.utils.asciiToHex(
            "Yea no, this guy is totes legit"
        );
        let hashedDataToSign3 = web3.utils.keccak256(
            web3.eth.abi.encodeParameters(
    ...
    it("Should proceed a batch of 2 transfers", async () => {
        ...
        let tx = await token.batchTransfer([user2, user3], [300, 200], {
            from: user1
        }).should.be.fulfilled;
        log(`Cumulative gas cost for 2 transfer transactions ${tx.receipt.gasUsed}`);
        let balance1 = await token.balanceOf(user1);
        let balance2 = await token.balanceOf(user2);
        let balance3 = await token.balanceOf(user3);
    ...
    it("Should proceed a batch of 20 transfers", async () => {
        ...
        let tx = await token.batchTransfer([user2, user2, user2, user2, user2, user2, user2, user2, user2, user2, user2, user2, user2, user2, user2, user2, user2, user2, user2, user2],
    ...
    it("Should proceed a batch of 2 forcedTransfer transactions", async () => {
        ...
        let tx = await token.batchForcedTransfer(
            [user1, user1],
            [user2, user3],
            [300, 200],
            {
    ...
    it("Should proceed a batch of 2 freezePartialTokens transactions", async () => {
        await token.transfer(user2, 300, {from: user1}).should.be.fulfilled;
        let tx = await token.batchFreezePartialTokens([user1, user2], [200, 200], {
            from: agent
        });
        log(`Cumulative gas cost for 2 freezePartialTokens transactions ${tx.receipt.gasUsed}`);
        let frozenTokens1 = await token.frozenTokens(user1);
        let frozenTokens2 = await token.frozenTokens(user2);
    ...
    it("Should proceed a batch of 20 freezePartialTokens transactions", async () => {
        await token.transfer(user2, 300, {from: user1}).should.be.fulfilled;
        let tx = await token.batchFreezePartialTokens([user1, user1, user1, user1, user1, user1, user1, user1, user1, user1,
                user2, user2, user2, user2, user2, user2, user2, user2, user2, user2],
            [20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20], {
                from: agent
            });
    ...
    it("Should proceed a batch of 2 unfreezePartialTokens transactions", async () => {
        ...
        let tx = await token.batchUnfreezePartialTokens([user1, user2], [200, 200], {
            from: agent
        });
    ...
    it("Should proceed a batch of 20 unfreezePartialTokens transactions", async () => {
        ...
        let tx = await token.batchUnfreezePartialTokens(
