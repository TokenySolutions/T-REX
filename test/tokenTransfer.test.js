...
contract("Token", accounts => {
  ...
  it("Should burn in batch", async () => {
    await token.batchMint([user1, user2], [1000, 1000], { from: agent }).should
      .be.fulfilled;
    await token.batchBurn([user1, user2], [1000, 1000], { from: agent }).should
      .be.fulfilled;
    let balance1 = await token.balanceOf(user1);
    let balance2 = await token.balanceOf(user2);
    balance1.toString().should.equal("1000");
  ...
  it("Should force transfer in batch", async () => {
	await identityRegistry.registerIdentity(user3, user3Contract.address, 91, {
		from: agent
	  }).should.be.fulfilled;
	  //user1 adds claim to identity contract
  ...
  it("Should partial freeze token in batch", async () => {
    await token.transfer(user2, 300, { from: user1 }).should.be.fulfilled;
  ...
  it("Should partial unfreeze token in batch", async () => {
    await token.transfer(user2, 300, { from: user1 }).should.be.fulfilled;
    await token.batchFreezePartialTokens([user1, user2], [200, 200], {
      from: agent
    });
