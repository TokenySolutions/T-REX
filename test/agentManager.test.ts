  ...
  describe(".callForceTransfer", () => {
    ...
    describe("when specified identity has the TransferManager role but the sender is not authorized for it", () => {
      it("should revert", async () => {
        ...
        await agentManager
          .connect(tokenAdmin)
          .addTransferManager(aliceIdentity.target);
  ...
  describe(".callPause", () => {
    describe("when specified identity is missing the Freezer role", () => {
      it("Should revert", async () => {
        ...
        await expect(
          agentManager.connect(aliceWallet).callPause(aliceIdentity.target)
    ...
    describe("when specified identity has the Freezer role but the sender is not authorized for it", () => {
      it("should revert", async () => {
        ...
        await expect(
          agentManager.connect(anotherWallet).callPause(aliceIdentity.target)
        ).to.be.revertedWithCustomError(agentManager, "SenderIsNotFreezer");
      });
    });
    ...
    describe("when identity has the Freezer role and the sender is authorized for it", () => {
      it("Should perform the pause", async () => {
        ...
        const pauseTx = await agentManager
          .connect(aliceWallet)
          .callPause(aliceIdentity.target);

        await expect(pauseTx)
          .to.emit(token, "Paused")
          .withArgs(agentManager.target);
  ...
  describe(".callUnpause", () => {
    describe("when specified identity is missing the Freezer role", () => {
      it("Should revert", async () => {
        ...
        await expect(
          agentManager.connect(aliceWallet).callUnpause(aliceIdentity.target)
        ).to.be.revertedWithCustomError(agentManager, "SenderIsNotFreezer");
      });
    });
    ...
    describe("when specified identity has the Freezer role but the sender is not authorized for it", () => {
      it("should revert", async () => {
        ...
        await expect(
          agentManager.connect(anotherWallet).callUnpause(aliceIdentity.target)
        ).to.be.revertedWithCustomError(agentManager, "SenderIsNotFreezer");
      });
    });
    ...
    describe("when identity has the Freezer role and the sender is authorized for it", () => {
      it("Should perform the pause", async () => {
        ...
        const pauseTx = await agentManager
          .connect(aliceWallet)
          .callUnpause(aliceIdentity.target);
  ...
  describe(".callMint", () => {
    describe("when specified identity is missing the SupplyModifier role", () => {
      it("Should revert", async () => {
        ...
        await expect(
          agentManager
            .connect(aliceWallet)
            .callMint(bobWallet.address, 1000, aliceIdentity.target)
        ).to.be.revertedWithCustomError(
          agentManager,
    ...
    describe("when specified identity has the SupplyModifier role but the sender is not authorized for it", () => {
      it("should revert", async () => {
        ...
        await agentManager
          .connect(tokenAdmin)
          .addSupplyModifier(aliceIdentity.target);
    ...
    describe("when identity has the SupplyModifier role and the sender is authorized for it", () => {
      it("Should perform the mint", async () => {
        ...
        await expect(mintTx)
          .to.emit(token, "Transfer")
          .withArgs(ethers.ZeroAddress, bobWallet.address, 1000);
      });
    });
  });
  ...
  describe(".callBatchMint", () => {
    ...
    describe("when identity has the SupplyModifier role and the sender is authorized for it", () => {
      it("Should perform the batch mint", async () => {
        ...
        await expect(mintTx)
          .to.emit(token, "Transfer")
          .withArgs(ethers.ZeroAddress, aliceWallet.address, 500);
      });
    });
  });
  ...
  describe(".callBurn", () => {
    describe("when specified identity is missing the SupplyModifier role", () => {
      it("Should revert", async () => {
        ...
        await expect(
          agentManager
            .connect(aliceWallet)
            .callBurn(bobWallet.address, 1000, aliceIdentity.target)
        ).to.be.revertedWithCustomError(
          agentManager,
          "SenderIsNotSupplyModifier"
        );
    ...
    describe("when specified identity has the SupplyModifier role but the sender is not authorized for it", () => {
      it("should revert", async () => {
        ...
        await agentManager
          .connect(tokenAdmin)
          .addSupplyModifier(bobIdentity.target);

        await expect(
          agentManager
            .connect(anotherWallet)
            .callBurn(bobWallet.address, 200, bobIdentity.target)
        ).to.be.revertedWithCustomError(
          agentManager,
          "SenderIsNotSupplyModifier"
        );
    ...
    describe("when identity has the SupplyModifier role and the sender is authorized for it", () => {
      it("Should perform the burn", async () => {
        ...
        await expect(burnTx)
          .to.emit(token, "Transfer")
          .withArgs(bobWallet.address, ethers.ZeroAddress, 200);
      });
    });
  });
  ...
  describe(".callBatchBurn", () => {
    ...
    describe("when identity has the SupplyModifier role and the sender is authorized for it", () => {
      it("Should perform the batch burn", async () => {
        ...
        await expect(burnTx)
          .to.emit(token, "Transfer")
          .withArgs(bobWallet.address, ethers.ZeroAddress, 500);
        await expect(burnTx)
          .to.emit(token, "Transfer")
          .withArgs(aliceWallet.address, ethers.ZeroAddress, 100);
      });
    });
  });
    ...
    describe("when identity has the Freezer role and the sender is authorized for it", () => {
      it("Should perform the freeze", async () => {
        ...
        await expect(tx)
          ...
          .withArgs(aliceWallet.address, true, agentManager.target);
  ...
  describe(".callBatchSetAddressFrozen", () => {
        ...
        await expect(
          agentManager
            ...
            .callBatchSetAddressFrozen(
              [aliceIdentity.target, bobWallet.address],
              [true, false],
              aliceIdentity.target
            )
        ).to.be.revertedWithCustomError(agentManager, "SenderIsNotFreezer");
    ...
    describe("when identity has the Freezer role and the sender is authorized for it", () => {
      it("Should perform the batch pause", async () => {
        ...
        await expect(pauseTx)
          .to.emit(token, "AddressFrozen")
          .withArgs(bobWallet.address, false, agentManager.target);
      });
    });
  });
  ...
  describe(".callRecoveryAddress", () => {
    ...
    describe("when specified identity has the RecoveryAgent role but sender is not authorized for it", () => {
      it("Should revert", async () => {
        ...
        await agentManager
          .connect(tokenAdmin)
          .addRecoveryAgent(aliceIdentity.target);
    ...
    describe("when identity has the RecoveryAgent role and the sender is authorized for it", () => {
      it("Should perform the recovery of the address", async () => {
        ...
        await bobIdentity
          ...
          .addKey(
            ethers.keccak256(
              abiCoder.encode(["address"], [anotherWallet.address])
            ),
            1,
            1
          );
  ...
  describe(".callRegisterIdentity", () => {
    ...
    describe("when specified identity has the WhiteListManager role but sender is not authorized for it", () => {
      it("Should revert", async () => {
        ...
        await agentManager
          .connect(tokenAdmin)
          .addWhiteListManager(aliceIdentity.target);
    ...
    describe("when identity has the WhitelistManager role and the sender is authorized for it", () => {
      it("Should perform the registration of the identity", async () => {
        ...
        await expect(registerTx)
          ...
          .withArgs(charlieWallet.address, charlieIdentity.target);
    ...
    describe("when identity has the WhitelistManager role and the sender is authorized for it", () => {
      it("Should perform the update of the identity", async () => {
        ...
        await expect(updateTx)
          ...
          .withArgs(bobIdentity.target, charlieIdentity.target);
      });
    });
  });
    ...
    describe("when specified identity is missing the WhiteListManager role", () => {
      it("Should revert", async () => {
        ...
        await expect(
          agentManager
            .connect(aliceWallet)
            .callUpdateCountry(bobWallet.address, 100, aliceIdentity.target)
        ).to.be.revertedWithCustomError(
          agentManager,
          "SenderIsNotWhiteListManager"
        );
    ...
    describe("when specified identity is missing the WhiteListManager role", () => {
      it("Should revert", async () => {
        ...
        await expect(
          agentManager
            .connect(aliceWallet)
            .callDeleteIdentity(bobWallet.address, aliceIdentity.target)
        ).to.be.revertedWithCustomError(
          agentManager,
          "SenderIsNotWhiteListManager"
        );
    ...
    describe("when identity has the WhitelistManager role and the sender is authorized for it", () => {
      it("Should perform the deletion of the identity", async () => {
        ...
        await expect(deleteTx)
          ...
          .withArgs(bobWallet.address, bobIdentity.target);
