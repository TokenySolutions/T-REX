...
describe("ClaimTopicsRegistry", () => {
  ...
  describe(".supportsInterface()", () => {
    it("should return false for unsupported interfaces", async () => {
      ...
      expect(
        await claimTopicsRegistry.supportsInterface(unsupportedInterfaceId)
      ).to.equal(false);
    });
    ...
    it("should correctly identify the IERC3643ClaimTopicsRegistry interface ID", async () => {
      ...
      const iClaimTopicsRegistryInterfaceId =
        await interfaceIdCalculator.getIERC3643ClaimTopicsRegistryInterfaceId();
      expect(
        await claimTopicsRegistry.supportsInterface(
          iClaimTopicsRegistryInterfaceId
        )
      ).to.equal(true);
    });
    ...
    it("should correctly identify the IERC173 interface ID", async () => {
      ...
      expect(
        await claimTopicsRegistry.supportsInterface(ierc173InterfaceId)
      ).to.equal(true);
    });
    ...
    it("should correctly identify the IERC165 interface ID", async () => {
      ...
      expect(
        await claimTopicsRegistry.supportsInterface(ierc165InterfaceId)
      ).to.equal(true);
    });
  });
});
