...
describe("IdentityRegistry", () => {
      ...
      it("should revert", async () => {
        ...
        await expect(
          identityRegistry
            ...
            .updateIdentity(bobIdentity.target, charlieIdentity.target)
    ...
    describe("when sender is not an agent", () => {
      it("should revert", async () => {
        ...
        await expect(
          identityRegistry
            .connect(anotherWallet)
            .updateCountry(bobIdentity.target, 100)
        ).to.be.revertedWithCustomError(
          identityRegistry,
          "CallerDoesNotHaveAgentRole"
        );
  ...
  describe(".isVerified()", () => {
    ...
    describe("when the claim issuer throws an error", () => {
      it("should return true if there is another valid claim", async () => {
        ...
        const trickyClaimIssuer =
          await ethers.deployContract("ClaimIssuerTrick");
        ...
        await trustedIssuersRegistry.addTrustedIssuer(
          trickyClaimIssuer.target,
          Array.from(claimTopics)
        );
        await trustedIssuersRegistry.addTrustedIssuer(
          claimIssuerContract.target,
          Array.from(claimTopics)
  ...
  describe(".disableEligibilityChecks()", () => {
    describe("when called by a non-owner", () => {
      it("should revert with OwnableUnauthorizedAccount", async () => {
        ...
        await expect(
          identityRegistry.connect(anotherWallet).disableEligibilityChecks()
        ).to.be.revertedWithCustomError(
          identityRegistry,
          "OwnableUnauthorizedAccount"
        );
    ...
    describe("when called by the owner", () => {
      it("should disable eligibility checks and allow all addresses to be verified", async () => {
        ...
        await expect(
          identityRegistry.connect(deployer).disableEligibilityChecks()
        ).to.emit(identityRegistry, "EligibilityChecksDisabled");
  ...
  describe(".enableEligibilityChecks()", () => {
    describe("when called by a non-owner", () => {
      it("should revert with OwnableUnauthorizedAccount", async () => {
        ...
        await expect(
          identityRegistry.connect(anotherWallet).enableEligibilityChecks()
        ).to.be.revertedWithCustomError(
          identityRegistry,
          "OwnableUnauthorizedAccount"
        );
    ...
    describe("when called by the owner after disabling", () => {
      it("should re-enable eligibility checks and enforce normal verification", async () => {
        ...
        await identityRegistry.connect(deployer).disableEligibilityChecks();
        await expect(identityRegistry.isVerified(anotherWallet.address)).to
          .eventually.be.true;

        await expect(
          identityRegistry.connect(deployer).enableEligibilityChecks()
        ).to.emit(identityRegistry, "EligibilityChecksEnabled");
  ...
  describe(".supportsInterface()", () => {
    it("should return false for unsupported interfaces", async () => {
      ...
      expect(
        await identityRegistry.supportsInterface(unsupportedInterfaceId)
      ).to.equal(false);
    });
    ...
    it("should correctly identify the IIdentityRegistry interface ID", async () => {
      ...
      const iIdentityRegistryInterfaceId =
        await interfaceIdCalculator.getIIdentityRegistryInterfaceId();
      expect(
        await identityRegistry.supportsInterface(iIdentityRegistryInterfaceId)
      ).to.equal(true);
    });
    ...
    it("should correctly identify the IERC173 interface ID", async () => {
      ...
      expect(
        await identityRegistry.supportsInterface(ierc173InterfaceId)
      ).to.equal(true);
    });
    ...
    it("should correctly identify the IERC165 interface ID", async () => {
      ...
      expect(
        await identityRegistry.supportsInterface(ierc165InterfaceId)
      ).to.equal(true);
    });
  });
});
