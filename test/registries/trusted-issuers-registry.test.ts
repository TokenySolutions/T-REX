...
describe("TrustedIssuersRegistry", () => {
    ...
    describe("when sender is the owner", () => {
      ...
      describe("when claim topics array have more that 15 elements", () => {
        it("should revert", async () => {
          ...
          await expect(
            trustedIssuersRegistry
              .connect(deployer)
              .updateIssuerClaimTopics(claimIssuerContract.target, claimTopics)
          ).to.be.revertedWithCustomError(
            trustedIssuersRegistry,
            "MaxClaimTopcisReached"
          );
      ...
      describe("when claim topics array is empty", () => {
        it("should revert", async () => {
          ...
          await expect(
            trustedIssuersRegistry
              .connect(deployer)
              .updateIssuerClaimTopics(claimIssuerContract.target, [])
          ).to.be.revertedWithCustomError(
            trustedIssuersRegistry,
      ...
      describe("when issuer is registered", () => {
        it("should update the topics of the trusted issuers", async () => {
          ...
          const tx = await trustedIssuersRegistry
            .connect(deployer)
            .updateIssuerClaimTopics(claimIssuerContract.target, [66, 100]);
          await expect(tx)
            .to.emit(trustedIssuersRegistry, "ClaimTopicsUpdated")
            .withArgs(claimIssuerContract.target, [66, 100]);

          await expect(
            trustedIssuersRegistry.hasClaimTopic(claimIssuerContract.target, 66)
          ).to.eventually.be.true;
          await expect(
  ...
  describe(".supportsInterface()", () => {
    it("should return false for unsupported interfaces", async () => {
      ...
      expect(
        await trustedIssuersRegistry.supportsInterface(unsupportedInterfaceId)
      ).to.equal(false);
    });
    ...
    it("should correctly identify the IERC3643TrustedIssuersRegistry interface ID", async () => {
      ...
      const iTrustedIssuersRegistryInterfaceId =
        await interfaceIdCalculator.getIERC3643TrustedIssuersRegistryInterfaceId();
      expect(
        await trustedIssuersRegistry.supportsInterface(
          iTrustedIssuersRegistryInterfaceId
        )
      ).to.equal(true);
    });
    ...
    it("should correctly identify the IERC173 interface ID", async () => {
      ...
      expect(
        await trustedIssuersRegistry.supportsInterface(ierc173InterfaceId)
      ).to.equal(true);
    });
    ...
    it("should correctly identify the IERC165 interface ID", async () => {
      ...
      expect(
        await trustedIssuersRegistry.supportsInterface(ierc165InterfaceId)
      ).to.equal(true);
    });
  });
});
