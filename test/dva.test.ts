...
describe("DVATransferManager", () => {
  async function deployFullSuiteWithTransferManager() {
    ...
    const transferManagerProxy = await ethers.deployContract(
      "DVATransferManagerProxy",
      [implementation.target, "0x8129fc1c"]
    );
    const transferManager = await ethers.getContractAt(
      "DVATransferManager",
      transferManagerProxy.target
    );
    return {
      ...context,
      suite: {
  ...
  async function deployFullSuiteWithVerifiedTransferManager() {
    ...
    await context.suite.token
      ...
      .addAgent(context.suite.transferManager.target);
    const identity = await context.suite.identityRegistry.identity(
  ...
  async function deployFullSuiteWithNonSequentialTransfer() {
    const context = await loadFixture(
      deployFullSuiteWithVerifiedTransferManager
    );
    await context.suite.transferManager
      .connect(context.accounts.deployer)
      .setApprovalCriteria(context.suite.token.target, true, true, false, [
        context.accounts.charlieWallet.address,
      ]);

    await context.suite.token
      .connect(context.accounts.aliceWallet)
      .approve(context.suite.transferManager.target, 100000);
    ...
    describe("when the contract is not initialized before", () => {
      it("should initialize", async () => {
        ...
        const implementation =
          await ethers.deployContract("DVATransferManager");
        const transferManagerProxy = await ethers.deployContract(
          "DVATransferManagerProxy",
          [implementation.target, "0x"]
        );
        const transferManager = await ethers.getContractAt(
          "DVATransferManager",
          transferManagerProxy.target
        ...
        await expect(
          transferManager.connect(context.accounts.deployer).initialize()
        ).to.eventually.be.fulfilled;
        await expect(transferManager.owner()).to.eventually.be.eq(
      ...
      describe("when DVA Manager is not an agent of the token", () => {
        it("should revert", async () => {
          ...
          await expect(
            ...
            context.suite.transferManager,
            `DVAManagerIsNotAnAgentOfTheToken`
          );
        });
      });
      ...
      describe("when DVA Manager is an agent of the token", () => {
        describe("when token is not already registered", () => {
          it("should modify approval criteria", async () => {
            const context = await loadFixture(
              deployFullSuiteWithTransferManager
            );
            await context.suite.token
              .connect(context.accounts.deployer)
              .addAgent(context.suite.transferManager.target);
            ...
            const approvalCriteria =
              await context.suite.transferManager.getApprovalCriteria(
                context.suite.token.target
              );
            await expect(tx)
        ...
        describe("when token is already registered", () => {
          it("should modify approval criteria", async () => {
            ...
            const previousApprovalCriteria =
              await context.suite.transferManager.getApprovalCriteria(
                context.suite.token.target
              );
            ...
            expect(approvalCriteria.hash.toString()).not.to.be.eq(
              previousApprovalCriteria.hash.toString()
            );
      ...
      describe("when sender has enough balance", () => {
        describe("when includeRecipientApprover is true", () => {
          it("should initiate the transfer with recipient approver", async () => {
            ...
            await context.suite.token
              ...
              .approve(context.suite.transferManager.target, 100000);
            const transferID =
              await context.suite.transferManager.calculateTransferID(
                0,
                context.accounts.aliceWallet.address,
                context.accounts.bobWallet.address,
                100
            ...
            await (await tx).wait();
            const transfer =
              await context.suite.transferManager.getTransfer(transferID);
            expect(transfer.approvers.length).to.be.eq(1);
            expect(transfer.approvers[0]["wallet"]).to.be.eq(
              context.accounts.bobWallet.address
            );
        ...
        describe("when additional approvers exist", () => {
          it("should initiate the transfer with token agent approver", async () => {
            ...
            expect(transfer.approvers[0]["approved"]).to.be.false;
            expect(transfer.approvers[1]["wallet"]).to.be.eq(
              context.accounts.anotherWallet.address
            );
        ...
        describe("when all criteria are enabled", () => {
          it("should initiate the transfer with all approvers", async () => {
            ...
            expect(transfer.approvers[1]["approved"]).to.be.false;
            expect(transfer.approvers[2]["wallet"]).to.be.eq(
              context.accounts.charlieWallet.address
            );
            expect(transfer.approvers[2]["approved"]).to.be.false;
            expect(transfer.approvers[3]["wallet"]).to.be.eq(
              context.accounts.anotherWallet.address
            );
            expect(transfer.approvers[3]["approved"]).to.be.false;

            const senderBalance = await context.suite.token.balanceOf(
              context.accounts.aliceWallet.address
            );
            expect(senderBalance).to.be.eq(1000);

            const frozenBalance = await context.suite.token.getFrozenTokens(
              context.accounts.aliceWallet.address
            );
  ...
  describe(".approveTransfer", () => {
    describe("when transfer does not exist", () => {
      it("should revert", async () => {
        ...
        await expect(
          context.suite.transferManager.approveTransfer(transferID)
        ).to.be.revertedWithCustomError(
          context.suite.transferManager,
          `InvalidTransferID`
        );
    ...
    describe("when transfer status is not pending", () => {
      it("should revert", async () => {
        const context = await loadFixture(
          deployFullSuiteWithNonSequentialTransfer
        );
        await context.suite.transferManager
          .connect(context.accounts.aliceWallet)
        ...
        await expect(
          context.suite.transferManager.approveTransfer(context.transferID)
        ).to.be.revertedWithCustomError(
          context.suite.transferManager,
    ...
    describe("when approval criteria are changed after the transfer has been initiated", () => {
      describe("when trying to approve before approval state reset", () => {
        it("should reset approvers", async () => {
          ...
          await (await tx).wait();
          const transfer = await context.suite.transferManager.getTransfer(
            context.transferID
          );
          expect(transfer.approvers.length).to.be.eq(1);
          expect(transfer.approvers[0]["wallet"]).to.be.eq(
      ...
      describe("when all parties approve the transfer", () => {
        it("should complete", async () => {
          ...
          await context.suite.transferManager
            ...
            .approveTransfer(context.transferID);
          await context.suite.transferManager
            .connect(context.accounts.bobWallet)
            .approveTransfer(context.transferID);
          const tx = context.suite.transferManager
            .connect(context.accounts.charlieWallet)
            .approveTransfer(context.transferID);
          ...
          const receiverBalance = await context.suite.token.balanceOf(
            context.accounts.bobWallet.address
          );
          expect(receiverBalance).to.be.eq(600);

          const dvaBalance = await context.suite.token.balanceOf(
            context.suite.transferManager.target
          );
    ...
    describe("when sequential approval is enabled", () => {
      describe("when caller is not the next approver", () => {
        it("should revert", async () => {
          const context = await loadFixture(
            deployFullSuiteWithSequentialTransfer
          );
          await expect(
            context.suite.transferManager
              .connect(context.accounts.charlieWallet)
  ...
  describe(".delegateApproveTransfer", () => {
    ...
    describe("when transfer status is not pending", () => {
      it("should revert", async () => {
        ...
        await expect(
          context.suite.transferManager.delegateApproveTransfer(
            context.transferID,
            [
              await signTransfer(
                context.transferID,
                context.accounts.charlieWallet
              ),
            ]
          )
        ).to.be.revertedWithCustomError(
          ...
          await expect(
            context.suite.transferManager.delegateApproveTransfer(
              ...
              [
                await signTransfer(
                  context.transferID,
                  context.accounts.anotherWallet
                ),
              ]
            )
          ).to.be.revertedWithCustomError(
  ...
  describe(".cancelTransfer", () => {
    describe("when transfer does not exist", () => {
      it("should revert", async () => {
        ...
        await expect(
          context.suite.transferManager.cancelTransfer(transferID)
        ).to.be.revertedWithCustomError(
          context.suite.transferManager,
          `InvalidTransferID`
        );
    ...
    describe("when transfer status is pending", () => {
      it("should cancel", async () => {
        ...
        const tx = await context.suite.transferManager
          ...
          .cancelTransfer(context.transferID);
        await expect(tx)
          .to.emit(context.suite.transferManager, "TransferCancelled")
          .withArgs(context.transferID);
  ...
  describe(".rejectTransfer", () => {
    describe("when transfer does not exist", () => {
      it("should revert", async () => {
        ...
        await expect(
          context.suite.transferManager.rejectTransfer(transferID)
        ).to.be.revertedWithCustomError(
          context.suite.transferManager,
          `InvalidTransferID`
        );
    ...
    describe("when transfer status is not pending", () => {
      it("should revert", async () => {
        ...
        await expect(
          context.suite.transferManager.rejectTransfer(context.transferID)
        ).to.be.revertedWithCustomError(
          context.suite.transferManager,
          `TransferIsNotInPendingStatus`
        );
  ...
  describe(".upgradeTo", () => {
    ...
    describe("when calling with owner account", () => {
      it("should upgrade proxy", async () => {
        ...
        await context.suite.transferManager
          .connect(context.accounts.deployer)
          .upgradeTo(newImplementation.target);

        // then
        const implementationAddress =
          await upgrades.erc1967.getImplementationAddress(
            context.suite.transferManager.target as string
          );
  ...
  describe(".getTransfer", () => {
    describe("when transfer does not exist", () => {
      it("should revert", async () => {
        ...
        await expect(
          context.suite.transferManager.getTransfer(transferID)
        ).to.be.revertedWithCustomError(
          context.suite.transferManager,
          `InvalidTransferID`
        );
  ...
  describe(".getNextApprover", () => {
    describe("when transfer does not exist", () => {
      it("should revert", async () => {
        ...
        await expect(
          context.suite.transferManager.getNextApprover(transferID)
        ).to.be.revertedWithCustomError(
          context.suite.transferManager,
          `InvalidTransferID`
        );
    ...
    describe("when transfer status is not pending", () => {
      it("should revert", async () => {
        ...
        await expect(
          context.suite.transferManager
            .connect(context.accounts.aliceWallet)
            .getNextApprover(context.transferID)
        ).to.be.revertedWithCustomError(
          context.suite.transferManager,
          `TransferIsNotInPendingStatus`
        );
    ...
    describe("when no one approved the transfer", () => {
      it("should return first approver", async () => {
        ...
        const { nextApprover, anyTokenAgent } =
          await context.suite.transferManager.getNextApprover(
            context.transferID
          );
  ...
  describe(".getApprovalCriteria", () => {
    describe("when token is not registered", () => {
      it("should revert", async () => {
        ...
        await expect(
          context.suite.transferManager.getApprovalCriteria(
            context.suite.token.target
          )
        ).to.be.revertedWithCustomError(
          context.suite.transferManager,
  ...
  describe(".name", () => {
    it("should return the name of the module", async () => {
      ...
      expect(await context.suite.transferManager.name()).to.be.equal(
        "DVATransferManager"
      );
    });
  });
  describe(".supportsInterface()", () => {
    it("should return false for unsupported interfaces", async () => {
      ...
      expect(
        await context.suite.transferManager.supportsInterface(
    ...
    it("should correctly identify the IDVATransferManager interface ID", async () => {
      ...
      const iDVATransferManagerInterfaceId =
        await interfaceIdCalculator.getIDVATransferManagerInterfaceId();
      expect(
        await context.suite.transferManager.supportsInterface(
          iDVATransferManagerInterfaceId
        )
      ).to.equal(true);
    });
