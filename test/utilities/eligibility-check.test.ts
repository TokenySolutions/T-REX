...
describe('EligibilityChecker', () => {
  it('should return false when the identity is registered with topics', async () => {
    ...
    const eligibilityChecker = await ethers.deployContract('EligibilityChecker');
    const results = await eligibilityChecker.verifiedCheckDetails(identityRegistry.target, charlieWallet.address);
  ...
  it('should return true after fixture', async () => {
    ...
    const eligibilityChecker = await ethers.deployContract('EligibilityChecker');
    const results = await eligibilityChecker.verifiedCheckDetails(identityRegistry.target, aliceWallet.address);
    expect(results.length).to.be.equal(1);
