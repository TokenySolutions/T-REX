...
describe("UtilityChecker.upgrateTo", () => {
  describe("when calling directly", () => {
    it("should revert", async () => {
      ...
      const proxy = await ethers.deployContract("UtilityCheckerProxy", [
        implementation.target,
        "0x8129fc1c",
      ]);
      const utilityChecker = await ethers.getContractAt(
        "UtilityChecker",
        proxy.target
      );
      ...
      await expect(
        utilityChecker.connect(aliceWallet).upgradeTo(newImplementation.target)
  ...
  describe("when calling with owner account", () => {
    it("should upgrade proxy", async () => {
      ...
      await utilityChecker
        .connect(deployer)
        .upgradeTo(newImplementation.target);

      const implementationAddress =
        await upgrades.erc1967.getImplementationAddress(
          utilityChecker.target as string
        );
