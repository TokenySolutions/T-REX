...
describe("InvestorCountryCapModule", () => {
  ...
  describe("Country Cap Management", () => {
    it("should revert when calling directly", async () => {
      ...
      await expect(
        complianceModule.setCountryCap(840, 100)
    ...
    it("should set country cap correctly", async () => {
      ...
      const tx = await compliance.callModuleFunction(
        new ethers.Interface([
          "function setCountryCap(uint16 _country, uint256 _cap)",
        ]).encodeFunctionData("setCountryCap", [countryCode, cap]),
        complianceModule.target
      );

      await expect(tx)
        .to.emit(complianceModule, "CountryCapSet")
        .withArgs(countryCode, cap);
    });
    ...
    it("should revert when setting cap lower than current count", async () => {
      ...
      await expect(
        compliance.callModuleFunction(
          new ethers.Interface([
            "function setCountryCap(uint16 _country, uint256 _cap)",
          ]).encodeFunctionData("setCountryCap", [countryCode, cap - 1]),
          complianceModule.target
        )
  ...
  describe("Bypassed Identity Management", () => {
    it("should add bypassed identity correctly", async () => {
      ...
      const tx = await compliance.callModuleFunction(
        new ethers.Interface([
          "function addBypassedIdentity(address _identity)",
        ]).encodeFunctionData("addBypassedIdentity", [aliceWallet.address]),
        complianceModule.target
      );
      await expect(tx)
        .to.emit(complianceModule, "BypassedIdentityAdded")
    ...
    it("should revert if trying to remove bypassed identity that is not bypassed", async () => {
      ...
      await expect(
        compliance.callModuleFunction(
          new ethers.Interface([
            "function removeBypassedIdentity(address _identity)",
          ]).encodeFunctionData("removeBypassedIdentity", [
            aliceWallet.address,
          ]),
          complianceModule.target
        )
    ...
    it("should remove bypassed identity correctly", async () => {
      ...
      const tx = await compliance.callModuleFunction(
        new ethers.Interface([
          "function removeBypassedIdentity(address _identity)",
        ]).encodeFunctionData("removeBypassedIdentity", [aliceWallet.address]),
        complianceModule.target
      );
      await expect(tx)
        .to.emit(complianceModule, "BypassedIdentityRemoved")
        .withArgs(aliceWallet.address);
    });
  });
  ...
  describe("Transfer Checks", () => {
    ...
    it("should enforce country cap", async () => {
      ...
      const aliceCountry = await identityRegistry.investorCountry(
        aliceWallet.address
      );
      await compliance.callModuleFunction(
        new ethers.Interface([
          "function setCountryCap(uint16 _country, uint256 _cap)",
        ]).encodeFunctionData("setCountryCap", [aliceCountry, 1]),
        complianceModule.target
      );

      await identityRegistry
        .connect(tokenAgent)
        .registerIdentity(charlieWallet.address, charlieIdentity, aliceCountry);
  ...
  describe("Wallet Management", () => {
    it("should enforce wallet per identity limit", async () => {
      ...
      await compliance.callModuleFunction(
        new ethers.Interface([
          "function setCountryCap(uint16 _country, uint256 _cap)",
        ]).encodeFunctionData("setCountryCap", [aliceCountry, 100]),
        complianceModule.target
      );
      ...
      await Promise.all(
        Array(21)
          ...
          .map(async () => {
            ...
            await identityRegistry
              .connect(tokenAgent)
              .registerIdentity(wallet.address, aliceIdentity, aliceCountry);
            ...
            await compliance.callModuleFunction(
              ...
              ]).encodeFunctionData("moduleMintAction", [wallet.address, 10]),
              complianceModule.target
            );
            return wallet;
          })
  ...
  describe("Country Change Handling", () => {
    it("should handle country change correctly", async () => {
      ...
      await compliance.callModuleFunction(
        new ethers.Interface([
          "function setCountryCap(uint16 _country, uint256 _cap)",
        ]).encodeFunctionData("setCountryCap", [newCountry, 10]),
        complianceModule.target
      );
      ...
      await identityRegistry
        .connect(tokenAgent)
        .updateCountry(aliceWallet.address, newCountry);
    ...
    it("should unbind the compliance", async () => {
      ...
      await compliance
        ...
        .callModuleFunction(
          new ethers.Interface([
            "function setCountryCap(uint16 _country, uint256 _cap)",
          ]).encodeFunctionData("setCountryCap", [840, 100]),
          module.target
        );
