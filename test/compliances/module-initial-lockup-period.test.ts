...
import { ModularCompliance, InitialLockupPeriodModule } from '../../typechain-types';
...
describe('InitialLockupPeriodModule', () => {
  ...
  async function deployInitialLockupPeriodModuleFullSuite() {
    ...
    const complianceModule = await ethers.getContractAt('InitialLockupPeriodModule', proxy.target);
    ...
    await token
      ...
      .burn(context.accounts.aliceWallet.address, await token.balanceOf(context.accounts.aliceWallet.address));
    await token
      .connect(context.accounts.tokenAgent)
      .burn(context.accounts.bobWallet.address, await token.balanceOf(context.accounts.bobWallet.address));
    ...
    return {
      ...
      accounts: {
        ...context.accounts,
        complianceSigner: await ethers.getImpersonatedSigner(context.suite.compliance.target.toString()),
      },
    };
  }
  ...
  describe('Initialization', () => {
    it('should initialize correctly', async () => {
      ...
      expect(await complianceModule.name()).to.equal('InitialLockupPeriodModule');
  ...
  describe('Transfer Checks', () => {
    it('should allow transfer after lockup period', async () => {
      ...
      expect(await complianceModule.connect(complianceSigner).moduleCheck(aliceWallet.address, bobWallet.address, 100, compliance.target)).to.be.true;
    });
    ...
    it('should prevent transfer during lockup period', async () => {
      ...
      await token.connect(tokenAgent).mint(aliceWallet.address, 100);
      expect(await complianceModule.moduleCheck(aliceWallet.address, bobWallet.address, 100, compliance.target)).to.be.false;
    ...
    describe('Transfer Checks with multiple lockup periods', () => {
      it('should allow transfer after lockup period', async () => {
        ...
        expect(await complianceModule.moduleCheck(aliceWallet.address, bobWallet.address, 200, compliance.target)).to.be.false;
        expect(await complianceModule.moduleCheck(aliceWallet.address, bobWallet.address, 100, compliance.target)).to.be.true;
