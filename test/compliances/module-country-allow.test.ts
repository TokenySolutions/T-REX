...
describe("CountryAllowModule", () => {
  ...
  describe(".canComplianceBind()", () => {
    it("should return true", async () => {
      const context = await loadFixture(deployComplianceWithCountryAllowModule);
      expect(
        await context.suite.countryAllowModule.canComplianceBind(
  ...
  describe(".owner", () => {
    it("should return owner", async () => {
      const context = await loadFixture(deployComplianceWithCountryAllowModule);
      await expect(
        context.suite.countryAllowModule.owner()
      ).to.eventually.be.eq(context.accounts.deployer.address);
    });
  });
  ...
  describe(".transferOwnership", () => {
    describe("when calling directly", () => {
      it("should revert", async () => {
        const context = await loadFixture(
          deployComplianceWithCountryAllowModule
        );
        await expect(
    ...
    describe("when calling with owner account", () => {
      it("should transfer ownership", async () => {
        ...
        // when
        const tx1 = await context.suite.countryAllowModule
        ...
        const tx2 = await context.suite.countryAllowModule
  ...
  describe(".upgradeTo", () => {
    ...
    describe("when calling with owner account", () => {
      it("should upgrade proxy", async () => {
        ...
        expect(await upgradedContract.isCountryAllowed(compliance.target, 42))
          .to.be.true;
      });
    });
  });
  ...
  describe(".batchAllowCountries()", () => {
    ...
    describe("when calling via the compliance contract", () => {
      it("should allow the given countries", async () => {
        ...
        await expect(tx)
          .to.emit(countryAllowModule, "CountryAllowed")
          .withArgs(compliance.target, 66);

        expect(await countryAllowModule.isCountryAllowed(compliance.target, 42))
          .to.be.true;
        expect(await countryAllowModule.isCountryAllowed(compliance.target, 66))
          .to.be.true;
      });
    });
  });
    ...
    describe("when calling via the compliance contract", () => {
      ...
      describe("when country is not allowed", () => {
        it("should allow the given country", async () => {
          ...
          expect(
            await countryAllowModule.isCountryAllowed(compliance.target, 42)
          ).to.be.true;
        });
      });
    });
  ...
  describe(".isComplianceBound()", () => {
    describe("when the address is a bound compliance", () => {
      it("should return true", async () => {
        ...
        await expect(countryAllowModule.isComplianceBound(compliance.target)).to
          .be.eventually.true;
      });
    });
    ...
    describe("when the address is not a bound compliance", () => {
      it("should return false", async () => {
        ...
        await expect(
          countryAllowModule.isComplianceBound(countryAllowModule.target)
        ).to.be.eventually.false;
      });
    });
  });
    ...
    describe("when sender is not a bound compliance", () => {
      it("should revert", async () => {
        ...
        await expect(
          countryAllowModule
            .connect(anotherWallet)
            .unbindCompliance(compliance.target)
        ).to.be.revertedWithCustomError(
          countryAllowModule,
          "OnlyBoundComplianceCanCall"
        );
  ...
  describe(".supportsInterface()", () => {
    it("should return false for unsupported interfaces", async () => {
      ...
      expect(
        await countryAllowModule.supportsInterface(unsupportedInterfaceId)
      ).to.equal(false);
    });
    ...
    it("should correctly identify the IModule interface ID", async () => {
      ...
      const iModuleInterfaceId =
        await interfaceIdCalculator.getIModuleInterfaceId();
      expect(
        await countryAllowModule.supportsInterface(iModuleInterfaceId)
      ).to.equal(true);
    });
    ...
    it("should correctly identify the IERC173 interface ID", async () => {
      ...
      expect(
        await countryAllowModule.supportsInterface(ierc173InterfaceId)
      ).to.equal(true);
    });
    ...
    it("should correctly identify the IERC165 interface ID", async () => {
      ...
      expect(
        await countryAllowModule.supportsInterface(ierc165InterfaceId)
      ).to.equal(true);
    });
  });
  describe(".addAndSetModule()", () => {
    ...
    describe("when calling batchAllowCountries not as owner", () => {
      it("should revert", async () => {
        ...
        await compliance
          .connect(deployer)
          .removeModule(countryAllowModule.target);
        ...
        await expect(
          compliance
            ...
            .addAndSetModule(countryAllowModule.target, [
              new ethers.Interface([
                "function batchAllowCountries(uint16[] calldata countries)",
              ]).encodeFunctionData("batchAllowCountries", [[42]]),
            ])
        ).to.be.revertedWithCustomError(
          compliance,
          "OwnableUnauthorizedAccount"
    ...
    describe("when providing more than 5 interactions", () => {
      it("should revert", async () => {
        ...
        await expect(
          compliance
            .connect(deployer)
            .addAndSetModule(countryAllowModule.target, interactions)
        ).to.be.revertedWithCustomError(compliance, "ArraySizeLimited");
      });
    });
      ...
      it("should add the module and interact with it", async () => {
        ...
        const tx = await compliance
          ...
          .addAndSetModule(countryAllowModule.target, [
            ...
            new ethers.Interface([
              "function batchAllowCountries(uint16[] calldata countries)",
            ]).encodeFunctionData("batchAllowCountries", [[66]]),
          ]);

        await expect(tx)
          .to.emit(compliance, "ModuleAdded")
          .withArgs(countryAllowModule.target);
        await expect(tx)
          .to.emit(countryAllowModule, "CountryAllowed")
          .withArgs(compliance.target, 42);
        await expect(tx)
  ...
  describe(".unbindCompliance()", () => {
    it("should unbind the compliance", async () => {
      const {
        suite: { countryAllowModule: module, compliance },
        accounts: { deployer },
      } = await loadFixture(deployComplianceWithCountryAllowModule);
      ...
      expect(await module.isCountryAllowed(compliance.target, 42)).to.be.equal(
        false
      );
    });
  });
  ...
  describe(".multicall functionality via callModuleFunction", () => {
    it("should execute multiple addAllowedCountry calls in a single transaction", async () => {
      ...
      const addCountry42Data = new ethers.Interface([
        "function addAllowedCountry(uint16 country)",
      ]).encodeFunctionData("addAllowedCountry", [42]);
      const addCountry43Data = new ethers.Interface([
        "function addAllowedCountry(uint16 country)",
      ]).encodeFunctionData("addAllowedCountry", [43]);
      ...
      const tx = await compliance
        ...
        .callModuleFunction(
          new ethers.Interface([
            ...
            [addCountry42Data, addCountry43Data],
          ]),
          countryAllowModule.target
        );
      ...
      expect(await countryAllowModule.isCountryAllowed(compliance.target, 42))
        .to.be.true;
      expect(await countryAllowModule.isCountryAllowed(compliance.target, 43))
        .to.be.true;
    ...
    it("should execute multiple different function calls in a single transaction", async () => {
      ...
      const removeCountry42Data = new ethers.Interface([
        "function removeAllowedCountry(uint16 country)",
      ]).encodeFunctionData("removeAllowedCountry", [42]);
    ...
    it("should revert if any call in multicall fails", async () => {
      ...
      const addCountry42AgainData = new ethers.Interface([
        "function addAllowedCountry(uint16 country)",
      ]).encodeFunctionData("addAllowedCountry", [42]);
      ...
      await compliance
        .connect(deployer)
        .callModuleFunction(addCountry42Data, countryAllowModule.target);
