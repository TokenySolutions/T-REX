...
describe("Compliance Module: TransferFees", () => {
  ...
  describe(".initialize", () => {
    it("should be called only once", async () => {
      ...
      const module = (
        await ethers.deployContract("TransferFeesModule")
      ).connect(deployer);
          ...
          await expect(
            context.suite.compliance.callModuleFunction(
              new ethers.Interface([
                "function setFee(uint256 _rate, address _collector)",
              ]).encodeFunctionData("setFee", [10001, collector]),
              context.suite.complianceModule.target
            )
        ...
        it("should revert", async () => {
          ...
          await expect(
            context.suite.compliance.callModuleFunction(
              new ethers.Interface([
                "function setFee(uint256 _rate, address _collector)",
              ]).encodeFunctionData("setFee", [1, collector]),
              context.suite.complianceModule.target
            )
          ).to.be.revertedWithCustomError(
            context.suite.complianceModule,
      ...
      describe("when collector address is verified", () => {
        it("should set the fee", async () => {
          ...
          await expect(tx)
            ...
            .withArgs(context.suite.compliance.target, 1, collector);

          const fee = await context.suite.complianceModule.getFee(
  ...
  describe(".canComplianceBind", () => {
    describe("when the module is not registered as a token agent", () => {
      it("should return false", async () => {
        ...
        const result = await complianceModule.canComplianceBind(
          context.suite.compliance.target
        );
      ...
      describe("when from and to belong to the same identity", () => {
        it("should do nothing", async () => {
          ...
          await context.suite.compliance.callModuleFunction(
            new ethers.Interface([
              "function setFee(uint256 _rate, address _collector)",
            ]).encodeFunctionData("setFee", [1000, collector]),
            context.suite.complianceModule.target
          );
          ...
          const collectedAmount =
            await context.suite.token.balanceOf(collector);
      ...
      describe("when fee is zero", () => {
        it("should do nothing", async () => {
          ...
          await context.suite.compliance.callModuleFunction(
            new ethers.Interface([
              "function setFee(uint256 _rate, address _collector)",
            ]).encodeFunctionData("setFee", [0, collector]),
            context.suite.complianceModule.target
          );
      ...
      describe("when caller is whitelisted", () => {
        it("should do nothing", async () => {
          ...
          await context.suite.compliance.callModuleFunction(
            ...
            ]).encodeFunctionData("setWhitelisted", [
              context.suite.compliance.target,
              true,
            ]),
            context.suite.complianceModule.target
  ...
  describe("Whitelist event and view functions", () => {
    describe("when caller address is whitelisted", () => {
      ...
      it("should get caller status", async () => {
        ...
        let isWhitelisted = await context.suite.complianceModule.isWhitelisted(
          context.suite.compliance.target,
          context.suite.compliance.target
        );
        ...
        isWhitelisted = await context.suite.complianceModule.isWhitelisted(
          context.suite.compliance.target,
          context.suite.compliance.target
        );
