...
describe("Compliance Module: ExchangeMonthlyLimits", () => {
  ...
  describe(".name()", () => {
    it("should return the name of the module", async () => {
      ...
      expect(await context.contracts.complianceModule.name()).to.be.eq(
        "ExchangeMonthlyLimitsModule"
      );
    });
  });
  ...
  describe(".initialize", () => {
    it("should be called only once", async () => {
      ...
      const module = (
        await ethers.deployContract("ExchangeMonthlyLimitsModule")
      ).connect(deployer);
  ...
  describe(".setExchangeMonthlyLimit", () => {
    describe("when calling directly", () => {
      it("should revert", async () => {
        ...
        await expect(
          context.contracts.complianceModule.setExchangeMonthlyLimit(
            exchangeID,
            1
          )
        ).to.revertedWithCustomError(
  ...
  describe(".getExchangeMonthlyLimit", () => {
    it("should return monthly limit", async () => {
      ...
      expect(
        await context.contracts.complianceModule.getExchangeMonthlyLimit(
          context.suite.compliance.target,
          exchangeID
        )
      ).to.be.eq(100);
    ...
    describe("when calling via compliance", () => {
      describe("when exchangeID is tagged", () => {
        it("should untag the exchangeID", async () => {
          ...
          const tx = context.contracts.complianceModule
            .connect(context.accounts.deployer)
            .removeExchangeID(exchangeID);
        ...
        describe("when sender is not a token agent", () => {
          describe("when the exchange monthly limit is not exceeded", () => {
            it("should increase exchange counter", async () => {
              const context = await loadFixture(
                deployExchangeMonthlyLimitsFullSuite
              );
              const from = context.accounts.aliceWallet.address;
              const to = context.accounts.bobWallet.address;
              const exchangeID =
              ...
              const counter =
                await context.suite.complianceModule.getMonthlyCounter(
                  context.suite.compliance.target,
                  exchangeID,
                  investorID
                );
          ...
          describe("when the exchange month is finished", () => {
            it("should set monthly timer", async () => {
              ...
              const timer =
                await context.suite.complianceModule.getMonthlyTimer(
                  context.suite.compliance.target,
                  exchangeID,
                  investorID
                );
  ...
  describe(".unbindCompliance()", () => {
    it("should unbind the compliance", async () => {
      ...
      expect(
        await module.getExchangeMonthlyLimit(compliance.target, exchangeID)
      ).to.be.equal(0);
    });
  });
});
