...
async function deploySupplyLimitFullSuite() {
  ...
  const SupplyLimitModule =
    await ethers.getContractFactory("SupplyLimitModule");
...
describe("Compliance Module: SupplyLimit", () => {
  ...
  describe(".initialize", () => {
    it("should be called only once", async () => {
      ...
      const module = (await ethers.deployContract("SupplyLimitModule")).connect(
        deployer
      );
    ...
    describe("when calling with owner account", () => {
      it("should upgrade proxy", async () => {
        ...
        const newImplementation =
          await ethers.deployContract("SupplyLimitModule");
  ...
  describe(".setSupplyLimit", () => {
    describe("when calling directly", () => {
      it("should revert", async () => {
        ...
        await expect(
          context.suite.complianceModule.setSupplyLimit(100)
        ).to.revertedWithCustomError(
          context.suite.complianceModule,
    ...
    describe("when calling via compliance", () => {
      it("should set supply limit", async () => {
        ...
        const tx = await context.suite.compliance.callModuleFunction(
          ...
          ]).encodeFunctionData("setSupplyLimit", [100]),
          context.suite.complianceModule.target
        );
    ...
    describe("when calling directly", () => {
      it("should return", async () => {
        ...
        await context.suite.compliance.callModuleFunction(
          new ethers.Interface([
            "function setSupplyLimit(uint256 _limit)",
          ]).encodeFunctionData("setSupplyLimit", [1600]),
          context.suite.complianceModule.target
        );
        const supplyLimit = await context.suite.complianceModule.getSupplyLimit(
          context.suite.compliance.target
        );
