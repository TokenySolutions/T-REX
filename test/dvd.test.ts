...
describe("DVDTransferManager", () => {
  ...
  async function deployFullSuiteWithTransferManagerAndInitiatedTransfer() {
    ...
    const event = receipt.logs.find(
      e => (e as EventLog)?.fragment?.name === "DVDTransferInitiated"
    ) as EventLog;
  ...
  describe(".initiateDVDTransfer()", () => {
    ...
    describe("when counterpart is zero address", () => {
      it("should revert", async () => {
        ...
        await erc20A
          .connect(charlieWallet)
          .approve(transferManager.target, 1000);
        await expect(
          transferManager
            .connect(charlieWallet)
            .initiateDVDTransfer(
    ...
    describe("when transfer was not initiated", () => {
      it("should revert", async () => {
        ...
        await expect(
          transferManager
            .connect(charlieWallet)
            .cancelDVDTransfer(ethers.ZeroHash)
        ).to.be.revertedWithCustomError(
          transferManager,
    ...
    describe("when transfer was not initiated", () => {
      it("should revert", async () => {
        ...
        await expect(
          transferManager
            .connect(charlieWallet)
            .takeDVDTransfer(ethers.ZeroHash)
        ).to.be.revertedWithCustomError(
          transferManager,
          "TransferIDDoesNotExist"
        );
  ...
  describe(".isTREXAgent()", () => {
    describe("when address is not a TREX agent", () => {
      it("should return false", async () => {
        ...
        expect(
          await transferManager.isTREXAgent(token.target, anotherWallet.address)
        ).to.be.false;
      });
    });
    ...
    describe("when address is a TREX agent", () => {
      it("should return true", async () => {
        ...
        expect(
          await transferManager.isTREXAgent(token.target, tokenAgent.address)
        ).to.be.true;
      });
    });
  });
  ...
  describe(".isTREXOwner()", () => {
    describe("when address is not a TREX owner", () => {
      it("should return false", async () => {
        ...
        expect(
          await transferManager.isTREXOwner(token.target, anotherWallet.address)
        ).to.be.false;
      });
    });
    ...
    describe("when address is a TREX agent", () => {
      it("should return true", async () => {
        ...
        expect(
          await transferManager.isTREXOwner(token.target, deployer.address)
        ).to.be.true;
      });
    });
  });
  ...
  describe(".isTREXToken()", () => {
    describe("When token does not have an Identity Registry (probably not a ERC3643 token", () => {
      it("should return false", async () => {
        ...
        expect(await transferManager.isTREX(transferManager.target)).to.be
          .false;
      });
    });
  });
