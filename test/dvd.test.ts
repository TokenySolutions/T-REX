...
describe("DVDTransferManager", () => {
  ...
  async function deployFullSuiteWithTransferManagerAndInitiatedTransfer() {
    ...
    await context.suite.erc20A
      ...
      .approve(context.suite.transferManager.address, 1000);
    ...
    await context.suite.erc20B
      ...
      .approve(context.suite.transferManager.address, 500);
    const tx = await context.suite.transferManager
      .connect(context.accounts.aliceWallet)
      .initiateDVDTransfer(
        context.suite.erc20A.address,
        1000,
  ...
  describe(".initiateDVDTransfer()", () => {
    ...
    describe("when counterpart is zero address", () => {
      it("should revert", async () => {
        ...
        await erc20A
          .connect(charlieWallet)
          .approve(transferManager.address, 1000);
        await expect(
          transferManager
            .connect(charlieWallet)
            .initiateDVDTransfer(
