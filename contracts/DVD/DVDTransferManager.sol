...
contract DVDTransferManager is Ownable {
  ...
  /**
     ...
     *  @param _counterpart the address of the counterpart, which will receive `_token1Amount` of `_token1` in exchange
     for
     ...
     *  requires `DVDTransferManager` contract to have the necessary allowance to process the DVD transfer on
     `msg.sender`
  ...
  function initiateDVDTransfer(
    ...
    require(
      balance >= _token1Amount,
      ERC20InsufficientBalance(msg.sender, balance, _token1Amount)
    );

    uint256 allowance = IERC20(_token1).allowance(msg.sender, address(this));
    require(
      allowance >= _token1Amount,
      ERC20InsufficientAllowance(msg.sender, allowance, _token1Amount)
    );
  ...
  function isTREX(address _token) public view returns (bool) {
    try IERC165(_token).supportsInterface(type(IERC3643).interfaceId) returns (
      bool _trex
    ) {
      return _trex;
    } catch {
