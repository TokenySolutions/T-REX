...
/// @dev Thrown when version of new implementation authority is not the same as the current implementation
///      authority version.
error VersionOfNewIAMustBeTheSameAsCurrentIA();

contract TREXImplementationAuthority is
  ...
  IERC165
{
  ...
  function fetchVersion(Version calldata _version) external override {
    ...
    require(
      _contracts[_versionToBytes(_version)].tokenImplementation == address(0),
  ...
  function useTREXVersion(Version calldata _version) public override onlyOwner {
    require(
      _versionToBytes(_version) != _versionToBytes(_currentVersion),
      VersionAlreadyInUse()
    );
    require(
      _contracts[_versionToBytes(_version)].tokenImplementation != address(0),
  ...
  function _versionToBytes(
    ...
    return
      bytes32(
        keccak256(
          abi.encodePacked(_version.major, _version.minor, _version.patch)
        )
      );
  }
}
