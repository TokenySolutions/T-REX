...
import "../registry/IGlobalIdentityRegistry.sol";
import "../roles/AgentRole.sol";

import "openzeppelin-solidity/contracts/ownership/Ownable.sol";

contract GlobalIdentityRegistry is IGlobalIdentityRegistry, AgentRole {
    ...
    /// mapping between a token and its ClaimTopicsRegistry Contract
    mapping(address => IClaimTopicsRegistry) private tokenTopicsRegistry;

    /// mapping between a token and its TrustedIssuersRegistry Contract
    mapping(address => ITrustedIssuersRegistry) private tokenIssuersRegistry;
    ...
    constructor (
        ...
        tokenTopicsRegistry[_token] = IClaimTopicsRegistry(_claimTopicsRegistry);
        tokenIssuersRegistry[_token] = ITrustedIssuersRegistry(_trustedIssuersRegistry);

        emit ClaimTopicsRegistrySet(_token, _claimTopicsRegistry);
        emit TrustedIssuersRegistrySet(_token, _trustedIssuersRegistry);
    }
    ...
    */
     ...
     */
    function issuersRegistry(address _token) public override view returns (ITrustedIssuersRegistry){
        return tokenIssuersRegistry[_token];
    }

    /**
     *  @dev Returns the ClaimTopicsRegistry linked to the current IdentityRegistry.
     */
    function topicsRegistry(address _token) public override view returns (IClaimTopicsRegistry){
        return tokenTopicsRegistry[_token];
    }
    ...
    */
    function isVerified(address _userAddress) public override view returns (bool) {
        require(address(tokenTopicsRegistry[msg.sender]) != address(0), "no claim topics registry for this token");
        require(address(tokenIssuersRegistry[msg.sender]) != address(0), "no trusted issuers registry for this token");
        if (address(identity[_userAddress]) == address(0)) {
            return false;
        }
        uint256[] memory claimTopics = tokenTopicsRegistry[msg.sender].getClaimTopics();
        ...
        for (claimTopic = 0; claimTopic < length; claimTopic++) {
            ...
            for (uint j = 0; j < claimIds.length; j++) {
                ...
                if (!tokenIssuersRegistry[msg.sender].isTrustedIssuer(issuer)) {
                    return false;
                }
                if (!tokenIssuersRegistry[msg.sender].hasClaimTopic(issuer, claimTopics[claimTopic])) {
                    return false;
                }
    ...
    */
    function setClaimTopicsRegistry(address _token, address _claimTopicsRegistry) public override onlyOwner {
        tokenTopicsRegistry[_token] = IClaimTopicsRegistry(_claimTopicsRegistry);
    ...
    */
    function setTrustedIssuersRegistry(address _token, address _trustedIssuersRegistry) public override onlyOwner {
        tokenIssuersRegistry[_token] = ITrustedIssuersRegistry(_trustedIssuersRegistry);
