  ...
  TokenPermit
{
  ...
  function pause() external override onlyAgent whenNotPaused {
    require(
      !getAgentRestrictions(msg.sender).disablePause,
      AgentNotAuthorized(msg.sender, "pause disabled")
    );
    _tokenPaused = true;
   ...
   */
  function setAgentRestrictions(
    address agent,
    TokenRoles memory restrictions
  ...
  function transferFrom(
    ...
    require(
      _amount <= balance,
      ERC20InsufficientBalance(_from, balance, _amount)
    );
    if (
  ...
  function recoveryAddress(
    ...
    require(
      ...
      AgentNotAuthorized(msg.sender, "recovery disabled")
    );
    require(balanceOf(_lostWallet) != 0, NoTokenToRecover());
    require(
      _tokenIdentityRegistry.contains(_lostWallet) ||
        _tokenIdentityRegistry.contains(_newWallet),
      RecoveryNotPossible()
    );
  ...
  function setAllowanceForAll(
    ...
    for (uint256 i = 0; i < targetsCount; i++) {
      require(
        ...
        DefaultAllowanceAlreadySet(_targets[i])
      );
      _defaultAllowances[_targets[i]] = _allow;
  ...
  function disableDefaultAllowance() external override {
    require(
      !_defaultAllowanceOptOuts[msg.sender],
      DefaultAllowanceAlreadyDisabled(msg.sender)
    );
  ...
  function enableDefaultAllowance() external override {
    require(
      _defaultAllowanceOptOuts[msg.sender],
      DefaultAllowanceAlreadyEnabled(msg.sender)
    );
  ...
  function identityRegistry()
    ...
    view
    override
    returns (IERC3643IdentityRegistry)
  {
  ...
  function transfer(
    ...
    require(
      _amount <= balance,
      ERC20InsufficientBalance(msg.sender, balance, _amount)
    );
    if (
      _tokenIdentityRegistry.isVerified(_to) &&
  ...
  function forcedTransfer(
    ...
    require(
      ...
      ERC20InsufficientBalance(_from, balanceOf(_from), _amount)
    );
  ...
  function burn(
    ...
    require(
      !getAgentRestrictions(msg.sender).disableBurn,
      AgentNotAuthorized(msg.sender, "burn disabled")
    );
    require(
    ...
    if (_amount > freeBalance) {
      ...
      _frozenTokens[_userAddress] =
        _frozenTokens[_userAddress] - (tokensToUnfreeze);
  ...
  function unfreezePartialTokens(
    ...
    require(
      ...
      AmountAboveFrozenTokens(_amount, _frozenTokens[_userAddress])
    );
    _frozenTokens[_userAddress] = _frozenTokens[_userAddress] - (_amount);
   ...
   */
  function getAgentRestrictions(
    address agent
  ) public view override returns (TokenRoles memory) {
  ...
  function name()
    public
    view
    override(IERC20Metadata, TokenPermit)
