...
contract DVATransferManager is
  IDVATransferManager,
  Initializable,
  ...
  IERC165
{
  ...
  function setApprovalCriteria(
    ...
    require(
      AgentRole(tokenAddress).owner() == msg.sender,
      OnlyTokenOwnerCanCall(tokenAddress)
    );
    require(
      AgentRole(tokenAddress).isAgent(address(this)),
      DVAManagerIsNotAnAgentOfTheToken(tokenAddress)
    );
  ...
  function initiateTransfer(
    ...
    require(
      approvalCriteria.hash != bytes32(0),
      TokenIsNotRegistered(tokenAddress)
    );

    IToken token = IToken(tokenAddress);
    require(
      token.identityRegistry().isVerified(recipient),
      RecipientIsNotVerified(tokenAddress, recipient)
    );
   ...
   */
  function delegateApproveTransfer(
    bytes32 transferID,
  ...
  function cancelTransfer(bytes32 transferID) external {
    Transfer storage transfer = _getPendingTransfer(transferID);
    require(
      msg.sender == transfer.sender,
      OnlyTransferSenderCanCall(transferID)
    );

    transfer.status = TransferStatus.CANCELLED;
    IToken(transfer.tokenAddress).unfreezePartialTokens(
      transfer.sender,
      transfer.amount
    );
  ...
  function rejectTransfer(bytes32 transferID) external {
    ...
    bool rejected = false;
    ApprovalCriteria memory approvalCriteria = _approvalCriteria[
    ...
    for (uint256 i = 0; i < transfer.approvers.length; i++) {
      ...
      require(
        ...
        ApprovalsMustBeSequential(transferID)
      );
    }
   ...
   */
  function getApprovalCriteria(
    address tokenAddress
  ) external view returns (ApprovalCriteria memory) {
    ApprovalCriteria memory approvalCriteria = _approvalCriteria[tokenAddress];
    require(
      approvalCriteria.hash != bytes32(0),
      TokenIsNotRegistered(tokenAddress)
  ...
  function calculateTransferID(
    ...
    bytes32 transferID = keccak256(
      abi.encode(_nonce, _sender, _recipient, _amount)
    );
    return transferID;
  }
  ...
  function _approvalCriteriaChanged(
    bytes32 transferID,
    Transfer storage transfer
  ...
  function _addApproversToTransfer(
    Transfer storage transfer,
    ApprovalCriteria memory approvalCriteria
  ) internal {
    ...
    for (uint256 i = 0; i < approvalCriteria.additionalApprovers.length; i++) {
      transfer.approvers.push(
        Approver(approvalCriteria.additionalApprovers[i], false, false)
      );
    }
  }
  ...
  function _getPendingTransfer(
    ...
    require(
      transfer.status == TransferStatus.PENDING,
      TransferIsNotInPendingStatus(transferID)
    );
  ...
  function _generateTransferSignatureHash(
    bytes32 transferID
