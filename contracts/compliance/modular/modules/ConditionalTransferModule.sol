...
contract ConditionalTransferModule is AbstractModuleUpgradeable {
  ...
  mapping(address compliance => mapping(uint256 nonce => mapping(bytes32 => uint256)))
    private _transfersApproved;
  ...
  function unApproveTransfer(
    ...
    uint256 nonce = getNonce(msg.sender);
    require(
      _transfersApproved[msg.sender][nonce][transferHash] > 0,
      TransferNotApproved(_from, _to, _amount)
    );
    _transfersApproved[msg.sender][nonce][transferHash]--;
    emit ApprovalRemoved(
      _from,
  ...
  function isTransferApproved(
    ...
    if (
      (
        (_transfersApproved[_compliance][getNonce(_compliance)])[_transferHash]
      ) > 0
    ) {
      return true;
    }
