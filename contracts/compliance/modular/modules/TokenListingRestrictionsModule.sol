...
contract TokenListingRestrictionsModule is AbstractModuleUpgradeable {
  ...
  mapping(address token => mapping(address investor => bool))
    private _tokenInvestorListingStatus;
  ...
  /**
     ...
     *  If it is ONCHAINID, the token will be listed for the ONCHAINID (it will be applied to all wallet addresses of
     the OID)
  ...
  function moduleCheck(
    ...
    bool listed = _tokenInvestorListingStatus[tokenAddress][_to] ||
      _tokenInvestorListingStatus[tokenAddress][
        _getIdentityByTokenAddress(tokenAddress, _to)
      ];
   ...
   *   they need to blacklist/disallow it.
   */
  function getTokenListingType(
    address _tokenAddress
  ) external view returns (ListingType) {
   ...
   */
  function getInvestorListingStatus(
    address _tokenAddress,
  ...
  function listToken(
    ...
    address investorAddress = _getInvestorAddressByAddressType(
      _tokenAddress,
      msg.sender,
      _addressType
    );
  ...
  function _getIdentityByTokenAddress(
    address _tokenAddress,
    address _userAddress
  ) internal view returns (address) {
    return
      address(IToken(_tokenAddress).identityRegistry().identity(_userAddress));
  }

  function _getBoundTokenAddress(
    address _compliance
  ) internal view returns (address) {
