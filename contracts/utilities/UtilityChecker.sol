...
import {
  IClaimIssuer,
  IIdentity
} from "@onchain-id/solidity/contracts/interface/IClaimIssuer.sol";
  ...
  UUPSUpgradeable
{
    ...
    )
  {
    ...
    ComplianceCheckDetails[] memory details = testTransferDetails(
      _token,
      _from,
      _to,
      _amount
  ...
  function testVerifiedDetails(
    ...
    IERC3643IdentityRegistry identityRegistry = IToken(_token)
      .identityRegistry();
    IERC3643TrustedIssuersRegistry tokenIssuersRegistry = identityRegistry
      .issuersRegistry();
    ...
    bytes memory data;
    uint256 topic;
    uint256[] memory requiredClaimTopics = identityRegistry
      .topicsRegistry()
      .getClaimTopics();
    ...
    for (uint256 claimTopic; claimTopic < topicsCount; claimTopic++) {
      ...
      IClaimIssuer[] memory trustedIssuers = tokenIssuersRegistry
      ...
      for (uint256 i; i < trustedIssuers.length; i++) {
        ...
        if (foundClaimTopic == topic) {
          bool pass;
          try
            IClaimIssuer(issuer).isClaimValid(identity, topic, sig, data)
  ...
  function testTransferDetails(
    ...
    IModularCompliance compliance = IModularCompliance(
      address(IToken(_token).compliance())
    );
